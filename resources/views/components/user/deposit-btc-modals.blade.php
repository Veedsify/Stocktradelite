      ...
      <form action="{{ route('deposit.btc', [$action['action'] ?? " upgrade"]) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-5">
          <div class="d-flex align-items-center">
            <input type="text" id="btc-address" class="form-control form-control-lg border py-3 border-primary"
              placeholder="Wallet Address" value="{{ $wallet->btc_address ?? ""}}">
            <button type="button" id="copy-button" class="btn btn-primary ms-2 h-100 px-3 py-3">
              <div class="d-flex align-items-center">
...
<script>
    document.getElementById('copy-button').addEventListener('click', function() {
      var btcAddress = document.getElementById('btc-address').value;
